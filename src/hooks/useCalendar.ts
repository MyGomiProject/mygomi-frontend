...
export const useWeeklyCalendar = (opts: { addressId?: number; year: number; month: number; isLoggedIn : boolean; }) => {
  const { addressId = DEFAULT_ADDRESS_ID, year, month, isLoggedIn } = opts;
  return useQuery<CalendarEvent[], Error>({
    ...
    staleTime: 1000 * 60,
    enabled: isLoggedIn && !!year && !!month, // 연/월 정보가 있을 때만 실행
  });
};